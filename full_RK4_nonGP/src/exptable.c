...
void make_ExpTable()
{
	...
	for(vindex=0;vindex<VNMAX;vindex++){
		...
		// Drug free part
		ina.Ta11[vindex] = 8.5539/(7.4392E-2*exp(-v/17.0)+2.0373E-1*exp(-v/150));
		ina.Ta12[vindex] = 8.5539/(7.4392E-2*exp(-v/15.0)+2.0373E-1*exp(-v/150));
		ina.Ta13[vindex] = 8.5539/(7.4392E-2*exp(-v/12.0)+2.0373E-1*exp(-v/150));
		ina.Tb11[vindex] = 7.5215E-2*exp(-v/20.3);
		ina.Tb12[vindex] = 2.7574*exp(-(v-5.0)/20.3);
		ina.Tb13[vindex] = 4.7755E-1*exp(-(v-10.0)/20.3);
		ina.Ta3[vindex]  = 5.1458E-6*exp(-v/8.2471);
		ina.Tb3[vindex]  = 6.1205*exp(v/13.542);
		ina.Ta2[vindex]  = 13.370*exp(v/43.749);
