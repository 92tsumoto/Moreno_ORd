...
void initial_mem()
{
	...
	if( ina.Tmss==NULL || ina.Ttaum==NULL 
		|| ina.Thss==NULL || ina.Ttauh_fast==NULL || ina.Ttauh_slow==NULL
		|| ina.Ttauj==NULL || ina.ThCaMKss==NULL ) exit(1);
	*/
	// var.fast INa from Moreno et al., Sci Trans Med
	ina.Ta11=(Number *)calloc(VNMAX,sizeof(Number));
	ina.Ta12=(Number *)calloc(VNMAX,sizeof(Number));
	ina.Ta13=(Number *)calloc(VNMAX,sizeof(Number));
	ina.Tb11=(Number *)calloc(VNMAX,sizeof(Number));
	ina.Tb12=(Number *)calloc(VNMAX,sizeof(Number));
	ina.Tb13=(Number *)calloc(VNMAX,sizeof(Number));
	ina.Ta3=(Number *)calloc(VNMAX,sizeof(Number));
	ina.Ta2=(Number *)calloc(VNMAX,sizeof(Number));
	ina.Tb3=(Number *)calloc(VNMAX,sizeof(Number));
	if( ina.Ta11==NULL || ina.Ta12==NULL || ina.Ta13==NULL
		|| ina.Tb11==NULL || ina.Tb12==NULL || ina.Tb13==NULL
		|| ina.Ta3==NULL || ina.Ta2==NULL || ina.Tb3==NULL ) exit(1);
	// ina_late
...
void closed_mem()
{
	...
	free(ina.Thss); free(ina.Ttauh_fast); free(ina.Ttauh_slow);
	free(ina.Ttauj); free(ina.ThCaMKss);
	*/
	free(ina.Ta11); free(ina.Ta12); free(ina.Ta13); 
	free(ina.Tb11); free(ina.Tb12); free(ina.Tb13);
	free(ina.Ta3); free(ina.Ta2); free(ina.Tb3); 
	...
	free(ito.TAi_fast); free(ito.TaCaMKss); if(var.celltype==1){free(ito.Tdepi);}
