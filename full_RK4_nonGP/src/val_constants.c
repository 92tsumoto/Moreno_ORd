...
void val_consts(FILE *fp1)
{
	...
	// Fast sodium current
		...
		//ina.Gna_fast = 15.0;	// (mS/uF).
		ina.Gna_fast = 20.0;	// (mS/uF).
		//ina.Ah_fast = 0.99;
		//ina.Ah_slow = 0.01; 	// (1.0-ina.Ah_fast)
		//ina.Ah_CaMK_fast = ina.Ah_fast;
		//ina.Ah_CaMK_slow = ina.Ah_slow;
		
	// For fast Ina (constants)
		ina.Tfac = 1.0/(pow(3,(37.0-(T-273))/10.0));
		
		ina.pH = 7.4;
		if(var.drug_type == 0){
			ina.pKa = 9.3; // for flecanide;
		} else if(var.drug_type==1){
			ina.pKa = 7.6; // for lidocaine;
		}
		ina.portion = 1/(1+pow(10,(ina.pH-ina.pKa)));
		if(var.drug_type == 0){
			ina.diffusion = 5500; // for flecanide;
		} else if (var.drug_type == 1){
			ina.diffusion = 500; // for lidocaine;
		}
		if(var.drug_type == 0){
			ina.kd0 = 11.2E-6; // for flecanide;
		}else if(var.drug_type ==1){
			ina.kd0 = 318.0E-6; // for lidocaine;
		}

		//ina.drug = 0.0E-6;
		ina.drug_charged = var.drug*ina.portion;
		ina.drug_neutral = var.drug*(1.0-ina.portion);

		// charged drug
		ina.kon = ina.drug_charged*ina.diffusion;
		ina.kcon = ina.kon;
		// neutral drug
		ina.k_on = ina.drug_neutral*ina.diffusion;
		ina.k_off = 400.0E-6*ina.diffusion;				
		ina.ki_on = ina.k_on*0.5;								
		ina.kc_on = ina.k_on*0.5;							
		if(var.drug_type == 0) { //Flecanide
			ina.ki_off = 5.4E-6*ina.diffusion;
			ina.kc_off = 800.0E-6*ina.diffusion;		
		}else if(var.drug_type ==1){ //Lidocaine
			ina.ki_off = 3.4E-6*ina.diffusion;
			ina.kc_off = 900.0E-6*ina.diffusion;		
		}
	...
	// Transient outward current
		if(var.celltype==0){
			...
			ito.Gto = 4.0*0.02;	// (mS/uF).
		}

	// L-type calcium current
		ical.Af_fast = 0.6;
		ical.Af_slow = 0.4;	// 1.0 - ical.Af_fast;
		ical.taujca = 75.0; // (ms).
